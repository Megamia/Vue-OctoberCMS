...
export default new (class CheckoutFormValidation {
    ...
    createOrder() {
        ...
        var data = {
            order: {
                payment_method_id: form
                    .find('input[name="payment_method_id"]:checked')
                    .val(),
                shipping_type_id: form
                    .find('input[name="shipping_type_id"]:checked')
                    .val(),
            },
            user: {
        ...
        $.request("MakeOrder::onCreate", {
            ...
            success: function (response) {
                if (
                    !!response &&
                    (response.status || !!response["X_OCTOBER_REDIRECT"])
                ) {
    ...
    markNotAvailableOfferPosition(response) {
        ...
        $(
            `.${this.cartItemItem}[data-position-id="${response.data.cart_position_id}"]`
